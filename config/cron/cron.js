        ...
        Mysql.ecp.query('select id from project.project where ?',prost,function(err,res){
            if(!err){
                ...
                Mysql.ecp.query('select id from ecp.course where ?',cpost,function(err,res){
                    ...
                    avalon(avatars,projects,courses);
                });
            }else{
                console.log(err);
            }
...
function avalon(avatars,projects,courses){
    ...
    var generateoneday = function(arr){
        arr.forEach(function(l,m){
            for(var loop=0;loop<l;loop++){
                daytime = moment().unix()+Math.round(3600*Math.random());
                //console.log(daytime);
                var name = getcname();
                            ...
                            if(!err){
                                ...
                                function perchase(){
                                    var spost = {
                                        ...
                                        userId:uid,
                                        remark:'f',
                                        createdTime:daytime+Math.round(1*daysecond*Math.random())
                                    };
...
function createschedule(uid,projectid){
    var data = {
        projectid: projectid,
        uid: uid
    };
...
//购买课程,创建后15天内购买
function perchase(uid,courses){
    var spost = {
        courseId:getcourseid(courses),
        userId:uid,
        remark:'f',
        createdTime:moment().unix()+Math.round(3600*Math.random())
    };
    var sql = 'insert into course_member set ?';
    Mysql.ecp.query(sql,spost,function(err,res){
        if(!err){
            ...
            Mysql.ecp.query('select id from course_lesson where courseId='+courseid,function(err,res){
                ...
                for(var i=1;i<=ran;i++){
                    ...
                    coupost= {
                        ...
                        lessonId:lessons[i-1],
                        status:'learning',
                        startTime:spost.createdTime+Math.round(1*3600*Math.random())
                    };
                    Mysql.ecp.query('insert into course_lesson_learn set ?',coupost,function(err,res){
                        console.log('用户',uid,'观看',coupost.courseId,'成功！');
                    });
...
var useraddops = function(){

    var ranusers = Math.ceil(Math.random()*3);
    //获取用户id
    var sql = 'SELECT id FROM `user` WHERE createdIp="221.224.10.50f" order by rand() limit '+ranusers;




    Mysql.ecp.query(sql,function(err,re){
        var users = re.map(function(i){
            return i.id;
        });
                ...
                Mysql.ecp.query('select id from ecp.course where ?',cpost,function(err,res){
                    if(!err){
                        ...
                        users.forEach(function(user){
                            ...
                            var ranlanch = Math.ceil(Math.random()*2);
                            for(var i=0;i<ranlanch;i++){
                                //随机启动项目
                                var ran = Math.floor((projects.length)*Math.random());
                                var projectid = projects[ran];

                                //启动项目
                                createschedule(user,projectid);
                            }

                            var ranwatch = Math.ceil(Math.random()*2);
                            for(var i=0;i<ranwatch;i++){
                                //购买课程同时随机观看
                                perchase(user,courses);
                            }
                        });
                        ...
                        //
                        //        //启动项目
                        //        createschedule(users[i],projectid);
                        //    }
                        //
                        //    var ranwatch = Math.ceil(Math.random()*2);
                        //    for(var i=0;i<ranwatch;i++){
                        //        //购买课程同时随机观看
                        //        perchase(users[i],courses);
                        //    }
                        //}
                    }else{
                        console.log(err);
                    ...
                    //};
                    //
                    //useraddoptions(users,projects,courses);
                });
            }else{
                console.log(err);
            }
