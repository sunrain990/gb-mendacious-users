...
function dooneday(){
    rsf.listPrefix('avatars', null, null, 105, function(err, ret) {
        ...
        var ranarray = Math.random();
        if(ranarray<0.4){
            array.push(1);
        }else if(ranarray<0.9&&ranarray>0.4){
            array.push(2);
        }else{
            var ranround = Math.round(10*Math.random());
            array.push(ranround);
        }
    ...
    var generateoneday = function(arr){
        arr.forEach(function(l,m){
            for(var loop=0;loop<l;loop++){
                daytime = startsecond+m*daysecond+Math.round(daysecond/24/6*Math.random());
                //console.log(daytime);
                var name = getcname();
...
function perchase(uid,courses){
    var spost = {
        ...
        userId:uid,
        remark:'f',
        createdTime:moment().unix()+Math.round(1800*Math.random())
    };
    var sql = 'insert into course_member set ?';
    Mysql.ecp.query(sql,spost,function(err,res){
        if(!err){
...
function getprojectid(projects){
                    ...
                    if(!err){
                        ...
                        users.forEach(function(user){
                            ...
                            for(var i=0;i<ranlanch;i++){
                                ...
                                //启动项目
                                createschedule(user,getprojectid(projects));
                            }
